...
function addPullOutContent() {
    index++
    var tb = $('.table-selfDefine tbody').find('.table-tr')
    tb.eq(index - 1).find('td').eq(0).text(index)
    tb.eq(index - 1).removeClass('hidden')
}
/*
清除申请表内容一行/
 */
function cleanRowPullOutContent(thisObj) {
    ...
    index--
    var tr = $('.table-selfDefine tbody').find('.table-tr')
